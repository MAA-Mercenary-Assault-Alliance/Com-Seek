...
<template>
  <div class="flex flex-grow flex-col w-full">
    ...
    <div id="content" class="flex flex-row space-x-10 h-full">
      ...
      <div v-if="!isLoading" id="boxes" class="flex flex-grow relative flex-col mt-10 space-y-6 w-4/5 xl:pr-42 pb-10">
        <template v-if="activeTab === 'companies'">
          <CompanyBox v-if="company_list" v-for="company in company_list" :key="company.UserID" :company-info="company" @refresh="getCompanies"/>
          <div v-else class="flex flex-col flex-grow items-center justify-center -translate-y-10">
            ...
            <span class="text-2xl text-gray-500 mt-10">No companies pending approval</span>
          </div>
        </template>
        <template v-if="activeTab === 'students'">
          <StudentBox v-if="student_list" v-for="student in student_list" :key="student.UserID" :student-info="student" @refresh="getStudents"/>
          <div v-else class="flex flex-col flex-grow items-center justify-center -translate-y-10">
            <img src="../../src/assets/leaf.svg" class="w-50" alt="leaf"/>
            <span class="text-2xl text-gray-500 mt-10">No students pending approval</span>
          </div>
        </template>
