...
async function getJob() {
  try {
    ...
    if (
      (error.response && error.response.data.error == "Invalid ID") ||
      error.response.data.error == "record not found"
    ) {
      router.back();
...
async function updateJob() {
  try {
    ...
    const res = await api.patch(`/job/${id.value}`, payload);
    ...
    <div id="input-field">
      <div class="grid grid-cols-2 space-y-5">
        <div class="job-input-box">
          <div class="title-and-error-box">
            ...
            <label v-if="errors.jobName" class="text-red-500 text-sm">{{
              errors.jobName
            }}</label>
          </div>
          <label class="input-box-gray">
            <input
              ...
              type="search"
              class="grow pl-3"
              placeholder="E.g. Account Manager (Sales Engineer)"
            />
          </label>
        </div>
        ...
        <div class="job-input-box">
          <div class="title-and-error-box">
            ...
            <label v-if="errors.location" class="text-red-500 text-sm">{{
              errors.location
            }}</label>
          </div>
          <label class="input-box-gray">
        ...
        <div class="job-input-box">
          <div class="title-and-error-box">
            ...
            <span v-if="errors.salaryMin" class="text-red-500 text-sm">{{
              errors.salaryMin
            }}</span>
            <span v-if="errors.salaryMax" class="text-red-500 text-sm">{{
              errors.salaryMax
            }}</span>
          </div>
          <div class="flex flex-row justify-start">
            ...
            <span class="inline-flex items-center justify-center px-5 text-xl">
              to
            </span>
            <label class="input-box-half">
              <input
        ...
        <div class="job-input-box">
          <div class="title-and-error-box">
            ...
            <label v-if="errors.jobType" class="text-red-500 text-sm">{{
              errors.jobType
            }}</label>
          </div>
        ...
        <div class="job-input-box">
          <div class="space-y-3">
            ...
            <span class="text-gray-400"
              >leave empty if require no experience</span
            >
            <div
              v-if="errors.expMin || errors.expMax"
              class="flex space-x-5 mt-3"
            >
              <span v-if="errors.expMin" class="text-red-500 text-sm">{{
                errors.expMin
              }}</span>
              <span v-if="errors.expMax" class="text-red-500 text-sm">{{
                errors.expMax
              }}</span>
            </div>
          </div>
      ...
      <div id="desc" class="mt-10">
        <div class="job-input-box">
          ...
          <span v-if="errors.jobDesc" class="text-red-500 text-sm">{{
            errors.jobDesc
          }}</span>
          <label>
            <textarea
              ...
              type="search"
              v-if="!preview"
              class="bg-white rounded-2xl border border-gray-300 placeholder-gray-300 grow pl-3 xl:w-5/6 w-full h-70 py-2"
              placeholder="E.g. Account Manager (Sales Engineer)"
            />
          </label>
        </div>
      ...
      <div class="flex flex-row space-x-10 mt-10 pb-20">
        <button
          v-if="!id"
          class="btn shadow-none border-0 h-15 rounded-3xl text-white text-lg font-extralight px-7 bg-[#44B15B]"
          @click="submitJob"
        >
          Confirm
        </button>
        ...
        <button
          class="btn shadow-none border-0 h-15 rounded-3xl text-white text-lg font-extralight px-7 bg-gray-300"
          @click="confirmBox.open()"
        >
          Cancel
        </button>
      </div>
