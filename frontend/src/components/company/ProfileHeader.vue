<template>
  <section class="relative">
    ...
    <div class="relative">
      ...
      />
      <div
        class="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-black/15 to-transparent"
      ></div>
    </div>

    <!-- Header card -->
    <div
      class="w-full mx-auto bg-white rounded-xl shadow-md -mt-12 p-6 relative"
    >
            ...
            <div>
              ...
              <div v-if="tagsList.length" class="flex flex-wrap gap-2 mt-2">
                <button
                  ...
                  :key="index"
                  type="button"
                  class="px-2 py-0.5 rounded-full border text-sm font-medium border-[#56A45C] text-[#0A3B1F] bg-[#EAF6EC] hover:bg-[#DDF0E1] transition"
...
const coverSrc = computed(() => props.profile?.cover || "/images/cover.png");
const avatarSrc = computed(() => props.profile?.avatar || "/images/avatar.png");
